...
['/storage/origin/Sonarr-TvShows-Seed' as File].eachMediaFolder{
	rename(folder:it, output: '/storage/destination/', format: "Tv Show/{n}/{'Season '+s}/{n} - {sxe} - {t}", action: "KEEPLINK", conflict: "OVERRIDE")
}

// Analyse and rename all movies
['/storage/origin/Radarr-Movies-Seed' as File].eachMediaFolder{
	rename(folder:it, output: '/storage/destination/', format: "Movies/Radarr/{ny}/{ny}", action: "KEEPLINK", conflict: "OVERRIDE")
}